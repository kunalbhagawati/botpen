"""``serve`` command: the Hub daemon - the single writer of the DB and the endpoint agents connect to.

Two channels in one asyncio loop:
...
class Hub:
    ...
    async def _apply_or_fail(self, row, owner_scaffold_id, peer_scaffold_id, tree, fn) -> dict:
        """Apply (or revoke) the ACL via the docker helper; close the log row accordingly."""
...
def serve() -> None:
    ...
    async def _reaper() -> None:
        ...
        while True:
            ...
            try:
                reaped = await asyncio.to_thread(docker_service.reap_stopped, settings.SCAFFOLD_TEARDOWN_AFTER_MINS)
                if reaped:
