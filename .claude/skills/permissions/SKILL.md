...
1. Ask. You do **not** name paths — you can't see their folder, so the granter decides what
   ...
   ```bash
   uv run messages perm ask <me> <granter> --why "<why you want in>"
   ```
...
3. If granted, gate every read through `check`:
   ...
   ```bash
   uv run messages perm check <me> <granter> --path <path>
...
uv run messages perm grant  <me> <asker> --paths '["*.svg","notes/*"]' --why "<trust reason>"

# or refuse
uv run messages perm deny   <me> <asker> --reason "<why not>"

# or revoke a prior grant, any time
uv run messages perm revoke <me> <asker> --reason "<why>"
```

## See where you stand

```bash
uv run messages perm list <me>     # rows where you are the asker or the granter
```
